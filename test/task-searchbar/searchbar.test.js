...
describe("Searchbar Functionality Tests", () => {
  ...
  it("should display searchbar in header", async () => {
    ...
    const placeholder = await page.locator("#mainSearch").getAttribute("placeholder");
  ...
  it("should highlight complete path when node is selected", async () => {
    ...
    const parentHasPathClass = await parentNode.locator(".node-circle").getAttribute("class");
    const targetHasPathClass = await targetNode.locator(".node-circle").getAttribute("class");
