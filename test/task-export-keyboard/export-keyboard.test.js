...
describe("Export Modal Keyboard Navigation Tests", () => {
  ...
  it("should open export modal with proper labels", async () => {
    ...
    const leftLabelStyle = await leftLabel.evaluate(el => getComputedStyle(el).userSelect);
    const rightLabelStyle = await rightLabel.evaluate(el => getComputedStyle(el).userSelect);
  ...
  it("should add items to export list with Right arrow", async () => {
    ...
    // Press Right arrow to add to export list
  ...
  it("should switch focus between search and export list with Tab", async () => {
    ...
    // Tab should switch focus to export list
  ...
  it("should handle mouse hover synchronization", async () => {
    ...
    if (resultCount > 1) {
      ...
      const selectedResults = await page.locator("#exportResults .search-result.selected").count();
